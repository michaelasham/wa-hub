/**
 * POST /api/view-session/click
 * Inject click at viewport coordinates (interactive view).
 * Requires session. Proxies to wa-hub POST /view-session/click.
 */
...
export async function POST(request: NextRequest) {
  ...
  const { token, x, y } = body;
  if (!token || x == null || y == null) {
    return NextResponse.json({ error: 'token, x, and y are required' }, { status: 400 });
  }
  try {
    const res = await fetch(`${BASE.replace(/\/$/, '')}/view-session/click`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${TOKEN}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ token, x: Number(x), y: Number(y) }),
    });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) {
      const err = (json as { error?: string }).error ?? (json as { message?: string }).message;
      return NextResponse.json({ error: err ?? 'Click failed' }, { status: res.status });
    }
